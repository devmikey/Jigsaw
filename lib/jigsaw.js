...
exports.createServer = function(routes) {
    ...
    **/
	...
	app.get('/jigsaw/services', function(req, res) {
        ...
        res.write(JSON.stringify(req.app.routePlugins));
        res.end();
	});
	
	app.get('/jigsaw/disable/*', function(req, res) {
		var plugins = req.app.routePlugins;
		for(var i = 0 ; i < plugins.length; i++){
			if (plugins[i].route == req.params[0]) {
				console.log("endpoint " + req.params[0] + " disabled");
				plugins[i].serviceStatus = "stopped";
			}
		}
	...
	app.get('/jigsaw/enable/*', function(req, res) {
		...
		for(var i = 0 ; i < plugins.length; i++){
			if (plugins[i].route == req.params[0]) {
				console.log("endpoint " + req.params[0] + " enabled");
				plugins[i].serviceStatus = "started";
			}
		}
    ...
    for(var i = 0; i < routes.length; i++) {
		...
		routes[i].serviceStatus = "started";
    }
